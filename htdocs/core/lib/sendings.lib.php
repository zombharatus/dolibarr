		...
		if ($num)
		{
			...
			while ($i < $num)
			{
				...
				if (! empty($conf->livraison_bon->enabled))
				{
					...
					$receiving='';
					if (!empty($expedition->linkedObjects['delivery']) && count($expedition->linkedObjects['delivery']) > 0) $receiving=reset($expedition->linkedObjects['delivery']);   // Take first link
